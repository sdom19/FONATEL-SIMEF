...
using System.Threading; // HSSFWorkbook, HSSFSheet


namespace GB.SUTEL.BL.Mantenimientos
{
    public class RegistroIndicadorBL : LocalContextualizer
    {        
        public RegistroIndicadorBL(ApplicationContext appContext)
            : base(appContext) {
                refDetalleAgrupacionAD = new DetalleAgrupacionAD(appContext);                
        }

        #region Atributos
        DetalleAgrupacionAD refDetalleAgrupacionAD;
        ...
        string IdOperador = "";        
        int whenInitIndexNextIndicador = 13;
        ...
        public Respuesta<Byte[]> CrearIndicadoresExcel(string IdOperador, string IdSolicitud, string nombreUsuario, string nombreExcel, string extension, string URLSaveFormatXLS)
        {
            ...
            try
            {
                ...
                Respuesta<List<DetalleAgrupacionSP>> objRespuesta = new Respuesta<List<DetalleAgrupacionSP>>();
                objRespuesta = refDetalleAgrupacionAD.TodosDetalleAgrupacionDetallado(this.IdOperador, IdSolicitud);

                if (objRespuesta.objObjeto.Count > 0)
                {
                    objRespuestRuta.objObjeto = initCrearExcel(objRespuesta.objObjeto, nombreExcel, extension, IdSolicitud, URLSaveFormatXLS);
                }
                else
                {
                    objRespuestRuta.strMensaje = "No existen datos para generar el excel. Intente más tarde.";
                }
            }
            catch (Exception ex)
            {
                objRespuestRuta.strMensaje = "Hubo un error al generar el archivo excel.";
        ...
        public Byte[] initCrearExcel(List<DetalleAgrupacionSP> objRespuesta, string nombreExcel, string extension, string IdSolicitud, string URLSaveFormatXLS)
        {
            ...
            {                
                ...
                this.setProtectionSheet(worksheet, 15, passwordExcel);
        ...
        public void setCellsExcel(ExcelWorksheet worksheet, List<DetalleAgrupacionSP> objRespuesta, ExcelWorksheet worksheetICoordenadasLectura)
        {
            ...
            for (int i = 0; i < objRespuesta.Count; i++)
            {
                if (nombreIndicador != objRespuesta[i].Nombre_Indicador)
                {
                    ...
                    fechaInicio = objRespuesta[i].Fecha_Inicio;
                    ...
                    objRespuestaFilterIndicador = this.enableTipoNivelDetalle(objRespuestaFilterIndicador);                    
                    ...
                    this.pintarEncabezadoIndicador(worksheet, nombreIndicador, nombreDesglose, nombreFrecuencia, nombresDireccion);
                    whenInitIndexNextIndicador = ends[1] + 6;
        ...
        public int[] FibonacciHijo(int? idPadre, Nullable<Guid> IdConstructorCriterioPadre, List<DetalleAgrupacionSP> objRespuesta, ExcelWorksheet worksheet, int indexObj, int indexLetra, int indexLetraParaDesgloseYFrecuencia)
        {
            ...
            foreach (DetalleAgrupacionSP da in objRespuesta)
            {
                if (idPadre == da.Id_Padre_Detalle_Agrupacion && IdConstructorCriterioPadre == da.Id_Padre_ConstructorCriterio)
                {
                    ...
                    this.setValueAndStyle(worksheet, cell, da.Nombre_Detalle_Agrupacion.ToString(), true);
            ...
            for (int i = 0; i < count; i++ )
            {
                ...
                if (objList[i].Tipo_Nivel_Detalle != null && padre == null)
                {
                    ...
                    foreach (string val in values)
                    {
                        ...
                        if (objList[i].Tabla_Tipo_Nivel_Detalle.Trim().ToUpper() == "PROVINCIA")
                        {
                        ...
                        switch (objList[i].Tabla_Tipo_Nivel_Detalle.Trim().ToUpper())
                        {
                            case "PROVINCIA":
                                ...
                                break;
                            case "CANTON":
                                idCanton = int.Parse(valuesIDTipoNivelDetalle[keyIdTipoNivelDetalle]);
                                break;
                            case "GENERO":
        ...
        {                          
            ...
            {                
                ...
                while (worksheet.Cells[cell].Value != null && indexEnd >= i)
                {
                    ...
                    objDetalleAgrupacionSP = this.consultarReglaParaIndicador(ListadoParcial, nombreAplicarRegla);
        ...
        public DetalleAgrupacionSP consultarReglaParaIndicador(List<DetalleAgrupacionSP> objRespuesta, string nombre)
        {
        ...
        public void aplicarReglaCellNumerico(ExcelWorksheet worksheet, string cell, int minValue, Int64 maxValue)
        {
            ...
            validation.Prompt = "Solamente números. Entre " + minValue + " a " + maxValue;
            validation.ShowInputMessage = true;
            validation.ErrorTitle = "Error de dato";
            validation.Error = "Solamente números. Entre " + minValue + " a " + maxValue;
        ...
        public void aplicarReglaDecimal(ExcelWorksheet worksheet, string cell, double minValue, double maxValue)
        {
            ...
            validation.ShowInputMessage = true;
            validation.ErrorTitle = "Error de dato";
            validation.Error = "Solamente números decimales. Entre " + minValue + " a " + maxValue;
            validation.ShowErrorMessage = true;
            validation.Operator = ExcelDataValidationOperator.between;
            validation.Formula.Value = minValue;
        ...
        public void pintarEncabezado(ExcelWorksheet worksheet, string nombreOperador, string nombreDireccion, string nombreServicio)
        {
            ...
            worksheet.Cells["G2"].Value = "Usuario:";
        ...
        public void pintarEncabezadoIndicador(ExcelWorksheet worksheet, string nombreIndicador, string nombreDesglose, string nombreFrecuencia, string nombresDireccion)
        {
            Color grayColorFromHex = System.Drawing.ColorTranslator.FromHtml("#e2efda");

            string cell = "A" + (this.whenInitIndexNextIndicador - 4).ToString();
        ...
        public void pintarFooterIndicador(ExcelWorksheet worksheet)
        {
            ...
            int indexLetra = 0;
            string cell = this.getLetras()[indexLetra] + (this.whenInitIndexNextIndicador - 7).ToString();
            while (worksheet.Cells[cell].Value != null)
            {
                for (int i = 5; i <= 6; i++)
                {
                ...
                indexLetra++;
                cell = this.getLetras()[indexLetra] + (this.whenInitIndexNextIndicador - 7).ToString();
            }
        ...
        #region obtenerArchivoExcel
        public Respuesta<Byte[]> obtenerArchivoExcel(string IdOperador, string IdSolicitud, string nombreUsuario, string nombreExcel, string extension)
        {
            Respuesta<Byte[]> objRespuestRuta = new Respuesta<Byte[]>();
            try
            {
                ...
                Respuesta<Byte[]> objRespuesta = new Respuesta<Byte[]>();
                objRespuesta = refDetalleAgrupacionAD.ObtenerArchivoExcel(this.IdOperador, IdSolicitud);

                if (objRespuesta.objObjeto != null)
                {
                    //objRespuestRuta.objObjeto = this.modificaNombreUsuarioExcel(objRespuesta.objObjeto, nombreUsuario);
                    objRespuestRuta.objObjeto = objRespuesta.objObjeto;
                }
                else
                {
                    objRespuestRuta.strMensaje = "No existen datos para generar el excel. Intente más tarde.";
            ...
            catch (Exception ex)
            {
                objRespuestRuta.strMensaje = "Hubo un error al generar el archivo excel.";
                objRespuestRuta.blnIndicadorTransaccion = false;
                objRespuestRuta.objObjeto = null;
                if (ex is CustomException) throw;
                string msj = GB.SUTEL.Shared.ErrorTemplate.InternalError;
                throw AppContext.ExceptionBuilder.BuildBusinessException(msj, ex);
            }
  ...
  public String actualizarEstadoDescargaExcels(string IdSolicitud, string IdOperador)
        {
            var resultado=refDetalleAgrupacionAD.actualizarEstadoDescargaExcel(IdSolicitud, IdOperador);

            return resultado;
        }

      /*  public bool obtieneEstadoDescargaExcel(string IdSolicitud, string IdOperador)
        {
            var resultado = refDetalleAgrupacionAD.obtenerEstadoDescargaExcel(IdSolicitud, IdOperador);
        ...
        {
            var resultado = refDetalleAgrupacionAD.actualizarEstadoCargaExcel(IdSolicitud, IdOperador);

            return resultado;
        }

        public Respuesta<bool> actualizarSolicitud(string IdSolicitud,string FechaIni,string FechaFin)
        {
            ...
            try
            {
                respuesta = refDetalleAgrupacionAD.actualizarFechasSolicitud(IdSolicitud, FechaIni, FechaFin);

                if (respuesta.blnIndicadorTransaccion)
                {
                    respuesta.strMensaje = "Se actualizaron las fechas de forma Correcta";
                }
            }
            catch (Exception ex)
            {
        ...
        /*  public bool obtieneEstadoCargaExcel(string IdSolicitud, string IdOperador)
          {
              var resultado = refDetalleAgrupacionAD.obtenerEstadoCargaExcel(IdSolicitud, IdOperador);

              return resultado;
          }*/
        public Respuesta<Byte[]> obtenerArchivoExcelPre( string IdSolicitud, string nombreExcel, string extension)
        {
            Respuesta<Byte[]> objRespuestRuta = new Respuesta<Byte[]>();
            try
            {
               ...
               // this.nombreUsuario = nombreUsuario;
                ...
                Respuesta<Byte[]> objRespuesta = new Respuesta<Byte[]>();
                objRespuesta = refDetalleAgrupacionAD.ObtenerArchivoExcelPre(IdSolicitud);
            ...
            {
                objRespuestRuta.strMensaje = "El Archivo aun no ha sido generado,";
                objRespuestRuta.blnIndicadorTransaccion = false;
                objRespuestRuta.objObjeto = null;
                if (ex is CustomException) throw;
                string msj = GB.SUTEL.Shared.ErrorTemplate.InternalError;
        ...
        #region modificaNombreUsuarioExcel
        public Byte[] modificaNombreUsuarioExcel(byte[] arrBytes, string nombreUsuario)
        {
            using (MemoryStream memStream = new MemoryStream(arrBytes))
            {
                ...
                ExcelWorksheet excelWorksheet = excelWorkBook.Worksheets["Indicadores"];

                excelWorksheet.Cells["H2"].Value = nombreUsuario;
                return package.GetAsByteArray();
            }
        }
        #endregion
