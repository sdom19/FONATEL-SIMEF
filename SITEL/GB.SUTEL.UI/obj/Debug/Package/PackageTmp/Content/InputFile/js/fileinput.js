...
(function ($) {
    ...
    var PREVIEW_TEMPLATE = '<div id="contenedorDePrevistas" class="file-preview {class}">\n' +
        '   <div class="file-preview-status text-center text-success"></div>\n' +
        '   <div class="close fileinput-Remover text-right">&times;</div>\n' +
        '   <div class="file-preview-thumbnails" ></div>\n' +
        '   <div class="clearfix"></div>' +
        '</div>';

    var CAPTION_TEMPLATE = '<div id="contenedorDeNombres" class="form-control file-caption {class}">\n' +
    ...
    FileInput.prototype = {
        ...
        listen: function () {
            ...
            self.$container.find('.fileinput-Remover').on('click', $.proxy(self.clear, self));
        },
        trigger: function (e) {
            var self = this;
        ...
        renderMain: function () {
            ...
            return self.mainTemplate.replace('{class}', self.mainClass).
                ...
                replace('{Subir}', self.renderUpload()).
                replace('{Remover}', self.renderRemove()).
                replace('{Buscar}', self.renderBrowse());
        },
        ...
        renderRemove: function () {
            var self = this, css = self.removeClass + ' fileinput-Remover fileinput-Remover-button', status = '';
            if (!self.showRemove) {
                return '';
            }
            if (self.isDisabled) {
        ...
        renderUpload: function () {
            ...
            if (isEmpty(self.uploadUrl)) {
                content = '<button id="btnSubir" onclick="mostrarModal()" type="button" class="' + self.uploadClass + '"' + status + '>' + self.uploadIcon + self.uploadLabel + '</button>';
            }
            else {
